...
export default function MainLayout({ children }: MainLayoutProps) {
    ...
    return (
        <div className="flex h-screen bg-[#F2F2F7] text-[#1C1C1E] font-sans overflow-hidden">
            {/* Sidebar */}
            <div className={clsx(
                "backdrop-blur-xl border-r border-black/10 flex flex-col z-20 shrink-0 transition-all duration-300",
            ...
            style={{ 
                ...
                <div className="p-6 pb-5 flex items-center justify-between">
                    {sidebarOpen && (
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="font-bold text-xl tracking-tight text-[#1C1C1E]">
                                    KONTEXT
                                </h1>
                ...
                {/* Navigation */}
                <nav className="flex-1 px-4 py-2 overflow-hidden flex flex-col">
                    {sidebarOpen && (
                    ...
                    <div className="flex-1 overflow-y-auto space-y-1">
                        {contexts.map((ctx) => (
                            <button
                                ...
                                className={clsx(
                                    "w-full text-left px-4 py-3 rounded-2xl flex items-center gap-3 transition-all active:scale-[0.98]",
                                )}
                                style={{
                            ...
                            >
                                ...
                                {sidebarOpen && (
                                    <span className="truncate font-semibold text-sm">{ctx.name}</span>
                                )}
                            </button>
                        ))}
                    </div>
                ...
                {/* Bottom Actions */}
                <div className="p-4 space-y-2 border-t border-black/10">
                    {sidebarOpen && (
                        <div className="p-4 rounded-3xl mb-2"
                             style={{ 
                               background: 'linear-gradient(135deg, #6B7A3E15 0%, #8B945625 100%)',
                             }}>
                          <div className="text-xs font-bold mb-1.5" style={{ color: '#6B7A3E' }}>
                            💡 Pro Tip
                          </div>
                    ...
                    <button
                        ...
                        className={clsx(
                            "w-full px-4 py-3 rounded-2xl flex items-center gap-3 transition-all active:scale-95",
                        ...
                        <>
                            ...
                            <button
                                ...
                                className={clsx(
                                    "w-full px-4 py-3 rounded-2xl flex items-center gap-3 transition-all active:scale-[0.98]",
                                    sidebarOpen ? "justify-start" : "justify-center",
                                    isActive('/team') ? "bg-white shadow-sm" : ""
                                )}
                                style={{
                                    color: isActive('/team') ? '#FF9500' : '#8E8E93'
                                }}
                            ...
                            <button
                                ...
                                className={clsx(
                                    "w-full px-4 py-3 rounded-2xl flex items-center gap-3 transition-all active:scale-[0.98]",
                                    sidebarOpen ? "justify-start" : "justify-center",
                                    isActive('/graph') ? "bg-white shadow-sm" : ""
                                )}
                                style={{
                                    color: isActive('/graph') ? '#FF9500' : '#8E8E93'
                                }}
                            ...
                            <button
                                onClick={() => navigate(`/dashboard/${activeContext.id}`)}
                                className={clsx(
                                    "w-full px-4 py-3 rounded-2xl flex items-center gap-3 transition-all active:scale-[0.98]",
                                    sidebarOpen ? "justify-start" : "justify-center",
                                    isActive('/dashboard') ? "bg-white shadow-sm" : ""
                                )}
                                style={{
                                    color: isActive('/dashboard') ? '#FF9500' : '#8E8E93'
                                }}
